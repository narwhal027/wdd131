...
// Function to search and sort hikes by name, description, and tags
function searchList(list, q) {
  function searchCallback(item) {
    return (
      item.name.toLowerCase().includes(q.toLowerCase()) ||
      item.description.toLowerCase().includes(q.toLowerCase()) ||
      item.tags.some(tag => tag.toLowerCase().includes(q.toLowerCase())) // Fix: Use some() instead of find()
    );
  }
  
  const filtered = list.filter(searchCallback);

  // Fix: Ensure sorting by distance works correctly
...
// Sorting simple list (Unrelated to hikes)
